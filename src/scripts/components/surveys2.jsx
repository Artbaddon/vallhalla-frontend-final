  ...
  // Function to show survey responses
  function showResponses(surveyId) {
    ...
    const surveyTitle = surveyCard.querySelector('.card-title').textContent;
    
    // Update modal title
    document.getElementById('responsesModalLabel').textContent = `Respuestas: ${surveyTitle}`;
    
    // Load and display survey responses
  ...
  // Function to load survey responses
  function loadSurveyResponses(surveyId) {
    ...
    const mockResponses = {
        ...
        questions: [
            {
                text: '¿Cómo calificaría el servicio general?',
                type: 'text',
                responses: [
                    'Excelente servicio',
                    'Muy bueno',
            ...
            {
                text: '¿Qué servicios utiliza con más frecuencia?',
                type: 'checkbox',
                options: ['Piscina', 'Gimnasio', 'Parqueadero', 'Seguridad'],
                responses: {
                    'Piscina': 30,
                    'Gimnasio': 25,
                    'Parqueadero': 40,
    ...
    // Update statistics
    document.getElementById('totalResponses').textContent = mockResponses.totalResponses;
    document.getElementById('responseRate').textContent = mockResponses.responseRate;
    document.getElementById('startDate').textContent = mockResponses.startDate;

    // Display responses
    const container = document.querySelector('.responses-container');
    container.innerHTML = '';

    mockResponses.questions.forEach((question, index) => {
        ...
        if (question.type === 'text') {
            ...
            // Display checkbox responses as a bar chart
        ...
        questionCard.innerHTML = `
            <div class="card-body">
                <h6 class="card-title">Pregunta ${index + 1}: ${question.text}</h6>
                ${responseContent}
            </div>
        `;
  ...
  function exportResponses(format) {
    ...
    Swal.fire({
        ...
        text: `Los datos se están exportando en formato ${format.toUpperCase()}`,
        icon: 'info',
        showConfirmButton: false,
        timer: 1500
    });
